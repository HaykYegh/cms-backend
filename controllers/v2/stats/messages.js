...
router.get('/', async (req, res) => {
  ...
  try {
    const result = await statsService.messages.getCount({ customerId, startDate, endDate });
...
/**
 * URL: /v2/stats/messages/types/count
 * METHOD: GET
...
router.get('/types/count', async (req, res) => {
  ...
  try {
    const result = await statsService
      .messages.types
      .getCount({ customerId, regionCode, startDate, endDate });
    res.json({ err: false, result });
  } catch (e) {
    logger.error(e);
    res.json({ err: true, err_msg: e.message });
...
/**
 * URL: /v2/stats/messages/types
 * METHOD: GET
 * Description: Get customer message by metric type (group and single)
 */
...
router.get('/types', async (req, res) => {
  ...
  try {
    const result = await statsService
      .messages.types
      .getRecords({ customerId, metricTypeId, regionCode, startDate, endDate });
    res.json({ err: false, result });
  } catch (e) {
    logger.error(e);
    res.json({ err: true, err_msg: e.message });
...
/**
 * URL: /v2/stats/messages/timeline
 * METHOD: GET
